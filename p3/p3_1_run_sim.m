...
for each = park_boundaries
    ...
    food = p2_food_source(X,Y);
    foods = [foods, food];
    parks = [parks, p2_park(X,Y, food)];
end
warning("on")
...
for i = 1:1 % do not exceed this many runs
    run_name = "regents_with_competition_massively_reduced_predators";
    warning("off")
    mkdir(sprintf("exports/%s", run_name));
    warning("on")
    [population, statistics] = p3_0_brood_and_predators(365 * 10, 50, 3, environment, start_park, run_name, true);

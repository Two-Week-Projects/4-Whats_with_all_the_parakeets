classdef p4_parakeet < handle
    ...
    properties
        ...
        id
        needs = struct(...
                "nourishment", 0,... % How hungry is the parakeet? (affected by food, flight, breeding, and affects breeding success)
                "environment", 0,... % How happy is the parakeet in its environment? (affected by parkness/buildingness and crampedness)
                "social", 0,... % How sociable/lonely is the parakeet? (affected by proximity to other parakeets, affects breeding success)
                "safety", 0)
        age = 0
        gender = "male"
        location = struct("x", 0, "y", 0)
        desired_location = struct("x", 0, "y", 0)
        mating = struct("paired_with", false, "successful", true, "shut_up_im_mating", false)
        mother = false;
        brood % A map of the resources around you
    end
    
    methods
        function self = p4_parakeet(age, gender, brood, location)
        ...
        function tick(self)
            ...
            % Increase needs by one day's worth
            self.needs.nourishment = min(100, self.needs.nourishment + (100 / parameters.needs.days_without_nourishment));
            self.needs.environment = min(100, self.needs.environment + (100 / parameters.needs.days_without_environment));
            self.needs.social = min(100, self.needs.social + (100 / parameters.needs.days_without_social));
            self.age = self.age - (1 / 365); % age the budgie by one day
            self.needs.safety = min(self.needs.safety - 1, 100);
            
            %% Check that you're not being born
            if self.age < 0.1151 % Go find your mother so that you can debirth yourself from her (you have 6 weeks)
                self.find_mother()
                if self.age <= 0
                    self.un_birth_yourself()
                end
            end
            ...
            if in_food_source
                ...
                self.needs.nourishment = max(self.needs.nourishment - have_food, 0);
            end
            if self.in_park()
                self.needs.environment = max(self.needs.environment -...
                    parameters.needs.boosters.environment, 0);
            end
            ...
            if tmp_nearest_parakeet ~= false
                ...
                if tmp_distance_to_nearest_parakeet <= parameters.needs.min_distances.socialising_min_distance
                    self.needs.social = max(self.needs.social - parameters.needs.boosters.social, 0);
                else
                    % dummy
                end
            end
        ...
        function un_birth_yourself(self)
            %% Unbirthing happens when a parakeet hits 0 years old.
            % They are removed from the brood completely, and the parents are succesfully done breading.
            if self.mother ~= false
                self.mother.mating.paired_with = false;
                self.mother.mating.successful = false;
            else
                fprintf("I was an OG parakeet!\n");
            end
            self.brood.remove_parakeet(self);            
        end
        function find_mother(self)
            %% Find the nearest female parakeet that is at least 3 years older than you (or fail to do so
            global parameters
            ignore_list = [];
            if self.mother == false
                for i = 1:length(self.brood.parakeets)
                    ...
                    if (nearest.age >= (parameters.life.breeding_age + self.age) && nearest.gender == "female")
                        self.mother = nearest;
                        self.mother.find_mate();
                        break
                    else
            ...
            if self.mother ~= false % different loop, because mother can be set immediately then moved to
                self.desired_location = self.mother.location;
                self.mother.find_mate(); % go find the father!
            end
        ...
        function un_mate(self) % I do not think this will be used.
            self.find_mate()

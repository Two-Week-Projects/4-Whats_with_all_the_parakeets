classdef p4_food_source < handle
    %P4_FOOD_SOURCE Summary of this class goes here
    ...
    methods
        function self = p4_food_source(X,Y)
            global parameters
            %P4_FOOD_SOURCE Construct an instance of this class
            %   Detailed explanation goes here
            self.X = X;
            self.Y = Y;
            shape = polyshape(X, Y);
        ...
        function tick(self)
            ...
            % Decrease food by a rate of 0.5% per day + 2000 (somewhat size dependant).
            self.contained_food = min(max((self.contained_food/1.005), 0), self.max_food);
            % n.b. parakeets add food back in to this. Not sure about this.....
        end
        ...
        function [is_food, give_food] = available_food(self)
            ...
            if self.has_food() == true
                ...
                self.contained_food = min(self.contained_food + give_food, self.max_food);                
            end

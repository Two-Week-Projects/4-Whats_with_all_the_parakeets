classdef p2_park < handle
    %P2_PARKS Summary of this class goes here
    ...
    methods
        function self = p2_park(X,Y, food_source)
            %P2_PARKS Construct an instance of this class
            %   Detailed explanation goes here
            self.X = X;
            self.Y = Y;
            self.shape = polyshape(X, Y);
            [self.centre.x, self.centre.y] = centroid(self.shape);
            self.competition = p3_competition(self, food_source);
        end

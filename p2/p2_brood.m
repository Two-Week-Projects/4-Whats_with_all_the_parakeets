classdef p2_brood < handle
    %P2_A_PARAKEET Summary of this class goes here
    ...
    methods
        function self = p2_brood(environment)
            %% Initialise brood
            ...
            self.statistics = p2_run_statistics();
        end
        function remove_parakeet(self, parakeet, death_note)
            global suppress_output
            if nargin < 3
                death_note = "unknown causes";
            end
            self.plotting.death_location.X = [self.plotting.death_location.X, parakeet.location.x];
            self.plotting.death_location.Y = [self.plotting.death_location.Y, parakeet.location.y];
            %% Remove a given parakeet from my list of parakeets
            switch death_note
                case "starvation"
                    self.statistics.day_statistics.death_stats.nourishment = self.statistics.day_statistics.death_stats.nourishment + 1;
                case "crippling loneliness"
                    self.statistics.day_statistics.death_stats.social = self.statistics.day_statistics.death_stats.social + 1;
                case "hating their environment"
                    self.statistics.day_statistics.death_stats.environment = self.statistics.day_statistics.death_stats.environment + 1;
                case "being attacked by a predator"
                    self.statistics.day_statistics.death_stats.predators = self.statistics.day_statistics.death_stats.predators + 1;
                case "old age"
                    self.statistics.day_statistics.death_stats.age = self.statistics.day_statistics.death_stats.age + 1;
            end
            if suppress_output ~= true
                fprintf("%s has died of %s. Rip.\n", parakeet.id, death_note);
            end
            self.statistics.day_statistics.total_deaths = self.statistics.day_statistics.total_deaths + 1;
        ...
        function spawn_parakeet(self, age, gender, location)
            %% Create a new parakeet and add to my list of parakeets
            if nargin < 4 % location not set 
                location = self.assure_location(...
                    self.environment.get_random_location());
            end
            % Make me a parakeet!
            new_parakeet = p2_parakeet(age, gender, self, self.assure_location(location));
            self.parakeets = [self.parakeets, new_parakeet];
            global suppress_output
            if suppress_output ~= true
                fprintf("Hooray, a new parakeet, %s, has been born!\n", new_parakeet.id);
            end            
        end
        function spawn_predator(self, age, gender, location)
            ...
            % Make me a predator!
            new_predator = p3_predators(age, gender, self, self.assure_location(location));
        ...
        function run_days(self, num_days, run_name)
            ...
            filename = sprintf('exports/%s/im_%s.gif', run_name, run_name);
            prev_annotates = [];
            %% Run the simulation for a specified number of days
            for each_day = start_date:start_date+num_days
                ...
                for each_parakeet = self.parakeets
                    ...
                    each_parakeet.tick();
                    if mod(each_day, 365) == 0
                        each_parakeet.mating = struct("paired_with", false, "successful", false, "shut_up_im_mating", false);
                    end
                    
                    if parameters.life.breeding_season(2) >= mod(each_day, 365) &&...
                            mod(each_day, 365) <= parameters.life.breeding_season(2)
                        each_parakeet.mate();
                    end
                end
                for each_predator = self.predators
                ...
                if self.plotting.init ~= false
                    ...
                    drawnow
%                     prev_annotates = p2_annotate_parameters(parameters, prev_annotates);
                    prev_annotates = p2_annotate_statistics(self.statistics, prev_annotates);
                    frame = getframe(gcf);
                    im = frame2im(frame);
                ...
                if isempty(self.parakeets) && each_day > 1
                    return
                end
            end
        end
